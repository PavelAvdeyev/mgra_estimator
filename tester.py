...
def run_PMAG(path, test_dir):
    ...
    if len(fnmatch.filter(os.listdir(old_path), '*.*')) > 0:
...
def run_MGRA(path, test_dir):
    ...
    if len(fnmatch.filter(os.listdir(current_path), '*.*')) > 0:
        operation = '~/tools/MGRA/build/mgra -c ' + \
                    current_path + 'sim.cfg -g ' + current_path + \
                    'blocks.txt -o ' + current_path
        print(subprocess.call(operation, shell=True))
...
def run_rococo(path, test_dir):
    ...
    tool_path = '/home/hamster/rococo/rococo-2.0.jar'
    operation = 'java -jar ' + \
                tool_path + ' -v -m=u -p=s ' + \
                'tree_tag.txt ' + \
                'blocks.txt result'
